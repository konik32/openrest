...
import orest.event.AfterCreateWithDtoEvent;
import orest.event.AfterSaveWithDtoEvent;
import orest.event.BeforeCreateWithDtoEvent;
import orest.event.BeforeSaveWithDtoEvent;
...
import org.springframework.data.rest.webmvc.config.PersistentEntityResourceWithDtoWrapper;
...
public class EntityDtoController implements ApplicationEventPublisherAware {
	...
	@Autowired
	public EntityDtoController(RepositoryRestConfiguration config, ConversionService defaultConversionService) {
		this.config = config;
	...
	public ResponseEntity<? extends ResourceSupport> putItemResource(RootResourceInformation resourceInformation,
		...
		return invoker.invokeFindOne(id) == null ? createAndReturn(objectToSave, resourceDtoWrapper.getDto(), invoker,
				assembler) : saveAndReturn(objectToSave, resourceDtoWrapper.getDto(), invoker, PUT, assembler);
	}
	...
	private ResponseEntity<ResourceSupport> createAndReturn(Object domainObject, Object dto, RepositoryInvoker invoker,
		...
		PersistentEntityResource resource = config.isReturnBodyOnCreate() ? assembler.toFullResource(savedObject)
				: null;
