...
public class ExpressionBuilder {
    ...
    public BooleanExpression createStaticFiltersExpression(PredicateContext predicateContext,
        ...
        for (ExpressionMethodInformation mInfo : staticFitlersMethods) {
            if (expressionEvaluator.checkCondition(mInfo.getStaticFilter().getCondition()))
                continue;
            BooleanExpression staticExp = (BooleanExpression) getExpression(mInfo.getMethod(), mInfo.getStaticFilter().getParameters(),
    ...
    public Expression create(ExpressionMethodInformation expressionMethodInformation, PredicateContext predicateContext,
    ...
    public BooleanExpression createSearchMethodExpression(ExpressionMethodInformation expressionMethodInformation,
            PredicateContext predicateContext, ExpressionEntityInformation expressionEntityInformation, String[] parameters) {
    ...
    private BooleanExpression processTreeRecursively(FilterPart part, PredicateContext predicateContext, Object expressionRepository) {
        if (part.getType() == FilterPartType.LEAF) {
            ...
            return (BooleanExpression) getExpression(methodInfo.getMethod(), part.getParameters(), expressionRepository, false);
        } else {
            BooleanExpression exp = null;
    ...
    private Object[] prepareParameters(Method method, String[] rawParameters, boolean processParameter) {
        ...
        for (int i = 0; i < rawParameters.length; i++) {
            ...
            if (!processParameter)
                result[i] = defaultConversionService.convert(parameterValue, TypeDescriptor.forObject(parameterValue), new TypeDescriptor(
                        param));
            else
