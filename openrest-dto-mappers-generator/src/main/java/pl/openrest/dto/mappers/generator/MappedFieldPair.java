...
public class MappedFieldPair {
    ...
    public CodeBlock toCreateCodeBlock() {
        if (dtoFieldInfo.isDto()) {
            CodeBlock getterCodeBlock = CodeBlockUtils.delegateCreateCodeBlock(entityFieldInfo.getType(),
                    CodeBlockUtils.getterCodeBlock(dtoFieldInfo.getGetterName()));
            return CodeBlockUtils.setterCodeBlock(entityFieldInfo.getSetterName(), getterCodeBlock);
        }
        return CodeBlockUtils
                .setterCodeBlock(entityFieldInfo.getSetterName(), CodeBlockUtils.getterCodeBlock(dtoFieldInfo.getGetterName()));
    }
    ...
    public CodeBlock toUpdateCodeBlock() {
        ...
        if (dtoFieldInfo.isDto()) {
            innerCodeBlock = CodeBlockUtils.delegateUpdateCodeBlock(dtoFieldGetterCodeBlock,
                    CodeBlockUtils.getterCodeBlock(CodeBlockUtils.ENTITY_PARAM_NAME, entityFieldInfo.getGetterName()));
        } else {
            innerCodeBlock = CodeBlockUtils.setterCodeBlock(entityFieldInfo.getSetterName(), dtoFieldGetterCodeBlock);
        }
        if (dtoFieldInfo.isNullable()) {
            return CodeBlockUtils.wrapWithNotNullOrNullableIf(dtoFieldGetterCodeBlock,
                    CodeBlockUtils.getterCodeBlock(dtoFieldInfo.getNullableGetterName()), innerCodeBlock);
        } else {
            return CodeBlockUtils.wrapWithNotNullIf(dtoFieldGetterCodeBlock, innerCodeBlock);
        }
    }
}
