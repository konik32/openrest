...
public class DefaultCreateAndUpdateMapper implements
	...
	public DefaultCreateAndUpdateMapper(DtoInformationRegistry dtoInfoRegistry,
			MapperDelegator mapperDelegator,
	...
	public Object create(Object from) {
		...
		if (dtoInfo.getType() == DtoType.MERGE)
			throw new IllegalStateException(
					"Cannot use merge dto to create entity");
	 ...
	 * @return
	 */
	private Object createByFields(final Object from,
			final DtoInformation dtoInfo) {
		final Object entity = org.springframework.data.util.ReflectionUtils
				.createInstanceIfPresent(dtoInfo.getEntityType().getName(),
						null);
		if (entity == null)
			throw new IllegalStateException(dtoInfo.getEntityType()
					+ " does not have default constructor");
		ReflectionUtils.doWithFields(from.getClass(), new FieldCallback() {
			@Override
			public void doWith(Field field) throws IllegalArgumentException,
					IllegalAccessException {
				Field entityField = ReflectionUtils.findField(
						dtoInfo.getEntityType(), field.getName());
				if (entityField == null)
					return;
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void doWithCollectionField(final Object from, final Field field,
			final Field entityField, final Object entity)
			throws IllegalArgumentException, IllegalAccessException {
		Collection<Object> fieldCollection = (Collection<Object>) field
				.get(from);
		Collection<Object> entityFieldCollection = CollectionFactory
				.createCollection(entityField.getType(), 0);
	 ...
	 * @throws IllegalAccessException
	 */
	private void populateEntityFieldCollection(Collection<Object> from,
		...
		while (it.hasNext()) {
			...
			if (object == null)
				continue;
			Object value = dtoInfoRegistry.contains(object.getClass()) ? mapperDelegator
	...
	public void merge(Object from, Object entity) {
		...
		if (dtoInfo.getType() == DtoType.CREATE)
			throw new IllegalStateException(
					"Cannot use create dto to merge with entity");
	...
	private void mergeSingleField(Field entityField, Field field,
			Object entity, Object value) throws IllegalArgumentException,
			IllegalAccessException {
			...
			if (subDtoInfo == null) {
				...
				if (entityFieldValue == null) {
					if (subDtoInfo.getType() == DtoType.MERGE)
						...
						setEntityField(entity, entityField,
								mapperDelegator.create(value));
				} else {
	...
	private void setEntityField(Object entity, Field entityField, Object value)
			throws IllegalAccessException, IllegalArgumentException {
		try {
			Method setter = ReflectionUtils.findMethod(entity.getClass(), "set"
					+ WordUtils.capitalize(entityField.getName()),
					entityField.getType());
			if (setter == null)
				throw new IllegalStateException("There is no setter for field "
						+ entityField.getName() + " in " + entity);
	...
	private boolean setNull(Object value, Field field, Object from)
			throws IllegalArgumentException, IllegalAccessException {
		if (value == null) {
			Nullable nullable = field.getAnnotation(Nullable.class);
			if (nullable == null)
				return false;
			Field isSetField = ReflectionUtils.findField(from.getClass(),
					nullable.value());
			if (isSetField == null)
				throw new IllegalStateException("There is no field "
						+ nullable.value() + " in " + from.getClass());
